...
def _tree_asof(panel, target_col, split, fit_predict, seed, model_name=None,
              imp_records=None, disease=None) -> pd.DataFrame:
    """Loop de horizontes as-of-origem; retorna long
    [horizon, cd_mun, nm_mun, time_idx, y_true, y_pred].

    Se `imp_records` (list) for passado, cada fit também extrai a importância
    real das features (gain-based) e acumula ali — permite reportar quais
    covariáveis o modelo mais usa, sem custo extra (a importância já sai do
    objeto ajustado, não precisa de treino/computação adicional)."""
    ...
    for h in split.horizons:
        ...
        if len(train) == 0 or len(test) == 0:
            continue
        preds, importances = fit_predict(train, test, cols, "_ytgt", seed)
        if imp_records is not None and model_name is not None:
            total = sum(importances.values()) or 1.0
            for feat_name, val in importances.items():
                imp_records.append({"model": model_name, "disease": disease, "origin": split.name,
                                    "horizon": h, "feature": feat_name, "importance": val,
                                    "importance_norm": val / total})
...
def _fp_lgbm(train, test, cols, ycol, seed):
    ...
    return m.predict(te[cols]), dict(zip(cols, m.feature_importances_))
...
def _fp_catboost(train, test, cols, ycol, seed):
    ...
    return m.predict(te[cols]), dict(zip(cols, m.get_feature_importance()))
...
def run_all_baselines(
    ...
    save_importance_path: str | None = None,
) -> pd.DataFrame:
    diseases = diseases or DISEASES
    records, long_rows, imp_records = [], [], []
    ...
    for d in diseases:
        ...
        for split in rolling_origin(origins, horizons):
            ...
            for m in ("lgbm", "catboost", "xgboost"):
                tl = _tree_asof(panel, target_col, split, _TREE_FP[m], seed,
                                model_name=m, imp_records=imp_records, disease=d)
                tree_long[m] = tl
    ...
    if save_importance_path:
        pd.DataFrame(imp_records).to_csv(save_importance_path, index=False)
...
if __name__ == "__main__":
    ...
    imp_path = PROJECT_ROOT / "reports" / "feature_importance_trees_raw.csv"
    (PROJECT_ROOT / "reports").mkdir(exist_ok=True)
    df = run_all_baselines(panel, save_long_path=str(long_path), save_importance_path=str(imp_path))
