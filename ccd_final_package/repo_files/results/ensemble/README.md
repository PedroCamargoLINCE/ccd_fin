...
> ⚠️ **DESATUALIZADO — não reflete o conjunto atual de 12 modelos.** Os números abaixo
> são de uma configuração **anterior** do pipeline: o ranking inclui `deepar` como se
> tivesse rodado normalmente, mas na run atual (PR #3, GPU) o DeepAR foi pulado por
> incompatibilidade de biblioteca (já corrigido — ver nota no `README.md` da raiz) e os
> modelos `lstm`, `gru`, `xgboost`, `prophet` nem existiam ainda quando este ensemble foi
> gerado. **Não cite estes números no artigo/relatório final.**
>
> **Para regenerar:** depois que `reports/` tiver as predições dos 12 modelos completos
> (incluindo o DeepAR corrigido — ver `run_deepar_only.py`), rode:
> ```bash
> python notebooks/ensemble.py
> ```
> Isso sobrescreve este arquivo e as tabelas/figuras de `results/ensemble/` com os
> números corretos e atuais.
...
## MAE médio no teste (configuração anterior — ver aviso acima)
...
## Ensemble vs melhor single (configuração anterior — ver aviso acima)
...
Ensemble vence em **2/20** pares nesta configuração anterior.
