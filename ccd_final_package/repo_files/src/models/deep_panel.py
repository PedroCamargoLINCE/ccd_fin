...
def train_deepar(df, cfg, train_end_time_idx, log_name):
    # FIX: DeepAR exige mesmas variaveis no encoder/decoder (fora o alvo).
    # Clima como unknown quebra isso -> usa recurrent=True (alvo+calendario, sem clima).
    training, val_es, test = make_datasets(df, cfg, train_end_time_idx, include_covariates=True, for_nb_loss=True, recurrent=True)
